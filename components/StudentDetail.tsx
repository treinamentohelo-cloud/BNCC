...
import { ArrowLeft, PlusCircle, Check, AlertTriangle, Clock, ClipboardCheck, X, Star, Calendar, History, Printer, TrendingUp, TrendingDown, BookOpen, LayoutList, User as UserIcon, Flag } from 'lucide-react';
...
// Tipos auxiliares definidos fora do componente para estabilidade
type TimelineEvent = 
...
export const StudentDetail: React.FC<StudentDetailProps> = ({
  ...
  // Group assessments by Subject (Mostrando sempre a mais recente para a habilidade)
  const assessmentsBySubject = skills.reduce((acc, skill) => {
    // Encontra a avaliação mais recente para esta habilidade
    ...
    if (assessment) {
         acc[skill.subject].push({ skill, assessment });
    }
    return acc;
  ...
  // Inject Remediation Events (Check valid dates)
  if (student.remediationEntryDate) {
    timelineEvents.push({ type: 'remediation_entry', date: student.remediationEntryDate });
  }
  if (student.remediationExitDate) {
    timelineEvents.push({ type: 'remediation_exit', date: student.remediationExitDate });
  }

  // Sort Descending (Safe Date parsing)
  const sortedTimeline: TimelineEvent[] = [...timelineEvents].sort((a, b) => {
  ...
  const formatDateTime = (dateString?: string) => {
      ...
      // If it looks like a full ISO string (has time), show it, otherwise just date
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between print:hidden">
        ...
        <button 
            onClick={handlePrint}
            className="flex items-center gap-2 px-4 py-2 bg-white border border-[#eaddcf] text-[#433422] rounded-xl hover:bg-[#fcf9f6] transition-colors font-medium shadow-sm"
        >
      ...
      <div className="flex gap-2 border-b border-gray-200 print:hidden">
          <button 
            ...
            className={`px-4 py-2 text-sm font-medium flex items-center gap-2 border-b-2 transition-colors ${viewMode === 'subjects' ? 'border-[#c48b5e] text-[#c48b5e]' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
          >
              <LayoutList size={16} /> Visão por Disciplina
          </button>
          <button 
            onClick={() => setViewMode('timeline')}
            className={`px-4 py-2 text-sm font-medium flex items-center gap-2 border-b-2 transition-colors ${viewMode === 'timeline' ? 'border-[#c48b5e] text-[#c48b5e]' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
          >
              <History size={16} /> Histórico Completo (Relatório)
          </button>
      </div>

      {/* CONTEÚDO: VISÃO POR DISCIPLINA */}
      {viewMode === 'subjects' && (
                ...
                <div key={subject} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden break-inside-avoid">
                    ...
                    {items.map(({ skill, assessment }) => {
                        ...
                        <div key={`${skill.id}-${assessment?.id}`} className="p-4 md:p-6 hover:bg-gray-50 transition-colors flex flex-col md:flex-row md:items-center justify-between gap-4">
                            ...
                            {assessment?.notes && (
                                <p className="mt-2 text-xs text-gray-600 bg-gray-50 p-2 rounded inline-block border border-gray-100 italic">
                                "{assessment.notes}"
                                </p>
                            )}
                            </div>
      ...
      {/* CONTEÚDO: HISTÓRICO COMPLETO (RELATÓRIO) */}
      {viewMode === 'timeline' && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div className="p-6 border-b border-gray-100 bg-gray-50 flex justify-between items-center print:bg-white print:border-b-2 print:border-black">
                  <div>
                    <h3 className="font-bold text-[#000039] text-lg">Relatório Geral do Aluno</h3>
                    <p className="text-xs text-gray-500">Linha do tempo de avaliações e movimentações do reforço escolar.</p>
                  </div>
                              ...
                              if (item.type === 'assessment') {
                                  ...
                                  return (
                                    <tr key={idx} className="hover:bg-gray-50 transition-colors">
                                        <td className="p-4 text-sm font-mono text-gray-600 whitespace-nowrap align-top">
                                            ...
                                            {assessment.term && (
                                                <div className="text-[10px] text-gray-400 mt-1 uppercase">{assessment.term}</div>
                                            )}
                                        </td>
                                        ...
                                        <td className="p-4 text-sm text-gray-600 italic align-top">
                                            {assessment.notes || '-'}
                                        </td>
                                    </tr>
                                  );
                                  ...
                                  return (
                                    <tr key={idx} className="bg-red-50/50 hover:bg-red-50 transition-colors border-l-4 border-red-400">
                                        <td className="p-4 text-sm font-mono text-red-700 whitespace-nowrap align-top font-bold">
                                            {formatDateTime(item.date)}
                                        </td>
                                        <td className="p-4 align-top">
                                            <div className="text-sm font-bold text-red-800 mb-1 flex items-center gap-2">
                                                <AlertTriangle size={14} /> Reforço Escolar
                                            </div>
                                            <div className="text-xs text-red-600">
                                                Início do ciclo de intervenção pedagógica.
                                            </div>
                                        </td>
                                        <td className="p-4 text-center align-top">
                                            <span className="inline-flex items-center gap-1 bg-red-100 text-red-700 px-2 py-1 rounded text-[10px] font-bold uppercase">
                                                Entrada
                                            </span>
                                        </td>
                                        <td className="p-4 text-sm text-red-700 italic align-top">
                                            O aluno apresentou dificuldades e foi encaminhado para reforço.
                                        </td>
                                    </tr>
                                  );
                                  ...
                                  return (
                                    <tr key={idx} className="bg-green-50/50 hover:bg-green-50 transition-colors border-l-4 border-green-400">
                                        <td className="p-4 text-sm font-mono text-green-700 whitespace-nowrap align-top font-bold">
                                            {formatDateTime(item.date)}
                                        </td>
                                        <td className="p-4 align-top">
                                            <div className="text-sm font-bold text-green-800 mb-1 flex items-center gap-2">
                                                <Flag size={14} /> Reforço Escolar
                                            </div>
                                            <div className="text-xs text-green-600">
                                                Conclusão do ciclo de intervenção.
                                            </div>
                                        </td>
                                        <td className="p-4 text-center align-top">
                                        ...
                                        <td className="p-4 text-sm text-green-700 italic align-top">
                                            O aluno superou as expectativas e atingiu os objetivos.
                                        </td>
                                    </tr>
                                  );
                              }
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        {[
                            ...
                            className={`border px-1 py-3 rounded-xl text-[10px] font-bold transition-all shadow-sm ${
                                status === opt.val ? 'ring-2 ring-[#c48b5e] ring-offset-1 ' + opt.color : 'border-gray-200 text-gray-500 hover:bg-gray-50'
                            }`}
                            >
                            {opt.label}
                            </button>
